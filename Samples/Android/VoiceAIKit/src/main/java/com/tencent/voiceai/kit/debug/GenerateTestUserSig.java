...
// VoiceAIKit - 计算测试用 UserSig（仅提供算法，不保存任何账号配置）

package com.tencent.voiceai.kit.debug;
 ...
 *
 *            The code may be able to calculate UserSig correctly, but it is only for quick testing
 *            of the SDK’s basic features, not for commercial applications.
...
public class GenerateTestUserSig {
     ...
     * 计算 UserSig（测试用）。
     * <p>
     * 本类不保存任何账号配置信息，sdkAppId 与 secretKey 均由调用者传入。
     *
     * @param sdkAppId   腾讯云 SDKAppID（由调用者提供）
     * @param userId     用户 ID
     * @param secretKey  计算 UserSig 所用的密钥（由调用者提供）
     * @return 有效的 UserSig 字符串；若 secretKey 为空或计算失败则返回空字符串
     */
    public static String genTestUserSig(long sdkAppId, String userId, String secretKey) {
